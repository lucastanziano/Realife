package com.realapp.realife.gui.viewpager;
...
public class AnimatedViewPager extends ViewPager {
	...
	public AnimatedViewPager(Context context, AttributeSet attrs) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AnimatedViewPager);
		int effect = ta.getInt(R.styleable.AnimatedViewPager_style, 0);
		String[] transitions = getResources().getStringArray(R.array.animated_viewpager_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.AnimatedViewPager_fadeEnabled2, false));
		setOutlineEnabled(ta.getBoolean(R.styleable.AnimatedViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.AnimatedViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
			setFadeEnabled(true);
