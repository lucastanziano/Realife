...
public class OutlineContainer extends FrameLayout implements Animatable {
	...
	private void init() {
		...
		mOutlinePaint.setStrokeWidth(ImageUtil.dpToPx(getResources(), 2));
		mOutlinePaint.setColor(getResources().getColor(R.color.holo_blue));
		mOutlinePaint.setStyle(Style.STROKE);

		int padding = ImageUtil.dpToPx(getResources(), 10);
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		int offset = ImageUtil.dpToPx(getResources(), 5);
		if (mOutlinePaint.getColor() != AnimatedViewPager.sOutlineColor) {
			mOutlinePaint.setColor(AnimatedViewPager.sOutlineColor);
		}
